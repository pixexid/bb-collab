// BB's official provider marks, vendored verbatim.
//
// Source: the installed app's frontend chunk
// `bb-app@0.37.0` → `app/dist/assets/plugin-sdk-hooks-CPZOXpqm.js`,
// whose provider resolver maps `codex`/`claude-code`/`pi` to these three icon
// components. Path data, `viewBox` and `fillRule` are copied exactly — do not
// redraw, re-round or re-fit them. `@bb/plugin-sdk/app` exports no provider
// logo component, so vendoring is the only way to draw the marks BB's own
// model selector draws; see docs/sidebar-provider-logos.md.
//
// Refresh procedure when BB updates: re-extract from the same chunk rather
// than editing these strings by hand.
...
const MARKS: Readonly<Record<string, ProviderMark>> = {
  "codex": {
    title: "OpenAI",
    viewBox: "0 0 24 24",
    fillRule: "evenodd",
...
// Vendor aliases resolve to the same official mark, so a provider-id rename
// does not silently drop a row to the fallback.
...
export function providerMark(providerId: unknown): ProviderMark | null {
