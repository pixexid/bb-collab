...
`INSTRUCTION_CONTRACT_VERSION: 49` — this is the instruction contract for agent sessions; apply the [instruction-contract version-bump test](docs/rules.md#version-bump-test) before changing it. The separate canonical `RUNTIME_CONTRACT_VERSION` lives in `src/foundation.ts` and feeds `contractDigest`.
