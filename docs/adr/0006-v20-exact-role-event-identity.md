...
per-turn limit. The independently validated exact completion sequence bounds
the walk; full pages must advance toward it. A missing completion, a short
page, or a non-advancing reader refuses `EXECUTION_COMPLETION_AMBIGUOUS`; a
truncated prefix is never accepted. The holder's earlier history is never read.
...
are measured directly from the SDK surface and state their own counts. The
GH-290 audit measured 138 completed spans: median 560, p95 2,570, maximum
4,394, with 12 spans beyond the former 2,048 ceiling. On the amended candidate,
that surface returned 18,428 events over sequence range
...
exact page boundaries, a measured 4,394-event span, missing completion, and
atomic partial-window refusal. The opt-in live-shape test is skipped by `npm run
