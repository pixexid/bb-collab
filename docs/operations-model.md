...
A seat may run `npm run --silent executed-profile -- --project PROJECT_ID --thread THREAD_ID` during its current turn. The reader correlates the active BB start to native records: Codex `~/.codex/sessions/YYYY/MM/DD/rollout-...-PROVIDER_SESSION_ID.jsonl` (`session_meta` plus the latest `turn_context`), Claude Code `~/.claude/projects/ENVIRONMENT_DERIVED_PATH/PROVIDER_SESSION_ID.jsonl` (latest non-synthetic assistant envelope), or Pi `~/.pi/agent/sessions/--ENVIRONMENT_DERIVED_PATH--/TIMESTAMP_SESSION_ID.jsonl` (latest assistant message and its reasoning-state ancestor). Cite the returned native source and values; if exact correlation or an element is absent, attest it as `UNKNOWN`.
