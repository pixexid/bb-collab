...
Adopt option 1 in two platform-gated phases. Rename the four request-sourced
values to `requested_model`, `requested_reasoning_level`,
`requested_permission_mode`, and `requested_service_tier`, and rename their
profile digest to requested provenance. Do not add or populate executed fields
...
| --- | ---: | ---: | --- |
| `work_item` | 45 | 0 | Reconstructed attempt history; it observed no profile |
| `role_holder` | 4 | 4 | Live role-context recording path |
| **Total** | **49** | **4** | 45 of 49 rows (91.8%) are backfilled and empty |
...
- `execution_attempts.actual_profile_digest`;
- `qualification_observations.executed_profile_digest`;
- `eligibility_projections.profile_digest`, including its primary key;
- `role_generations.holder_executed_profile_digest`.

The role-holder `attempt_digest` covers that value under the
`actualProfileDigest` key and also covers the holder-context digest. Qualification
...
Historical digests should not be silently recomputed in place. The migration
design should preserve their bytes as a versioned legacy/request digest shape,
rename every semantic consumer in the graph, and use a request-domain digest
...
move the four populated values and `actual_profile_digest` to requested names;
rename or type the qualification, eligibility, and role-generation digest
consumers as request-derived legacy evidence; preserve/version the existing
four `attempt_digest` values; and update every verifier, export, receipt, and
...
This note specifies no schema or runtime change. The migration must wait until
PR #340, which currently edits both `src/foundation.ts` and `server.ts`, is no
longer in flight. Its implementation needs a separately frozen migration work
order covering the full digest graph and live dry-run evidence.
