...
The check is per-change: name the deployed revision, establish containment and supervision — equality with `main` is not the test. "Merged, not deployed" is a complete and honest terminal status. V2 release activation is orchestrator-run under standing authorization through `activate-release.mjs`; its host-local receipt, loaded-root/entry proof, post-settle hashes, health, and schema digest are the deployment evidence. The canonical schema fingerprint alone decides ceremony: an unchanged fingerprint needs no routine backup, export, lane lull, or cross-project canary; a changed fingerprint requires governed backup and quiescence evidence before any binding changes.

Generated bundles are release artifacts, not committed source-review authority. CI builds an exact-head inactive candidate with the pinned toolchain and publishes its manifest and digest, including the candidate-owned runtime dependency closure for every emitted server external; source verification may generate ignored output but stays clean. Activation verifies that closure from the staged wrapper before importing schema or changing a binding. Only the receipt-bound activation transaction may stage, bind, and load it. A Git comparison of generated files, a successful reload, or a plugin-list row is not loaded-authority proof.
