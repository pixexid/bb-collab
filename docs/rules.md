...
Every pull request carries exactly one disposition line: `Closes #NN` plus `Acceptance: complete`, or `Related GH-NN` naming an OPEN issue, or `No issue:` followed by a reason. Run both canonical body gates before pushing: `scripts/pr-lifecycle.mjs` for disposition and `scripts/check-review-tier.mjs` for exactly one `Review tier: A|B|C` line. Tier A is forced by any Tier-A path (`server.ts`, `foundation.ts`, `app.tsx`, `AGENTS.md`, `dist/`, listed verify/lifecycle files, listed `scripts/*.mjs`, specific `docs/*.md` files) or authority/approval/atomicity/concurrency/cutover/migration/provenance/receipt/spend keyword; Tier C applies only when every path is under `docs/` or `tests/`, or is `README.md`/`AGENTS.md`. Because `dist/` is Tier A, a docs-only `docs/rules.md` edit that regenerates `dist/role-briefs.json` requires Tier A.
