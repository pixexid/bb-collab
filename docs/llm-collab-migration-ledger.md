...
- `MIGRATE`: a surviving mechanism or invariant belongs in bb-collab, either in
  an existing contract/issue or in a proposed missing BB-native queue item.
...
- `DUPLICATE-ALREADY-MIGRATED`: the surviving mechanism is already represented
  by the founding contract, canonical queue, or closed bb-collab issue; do not open a
...
- [GH-787 — Role lease and epoch store: one supervisor lease, one orchestrator lease per project, monotonic epochs](https://github.com/pixexid/llm-collab/issues/787) — **MIGRATE** — split the v1 monotonic governorship/manual role-generation requirement from renewable lease and automatic-failover behavior. **Order:** v1 maps to the [v1 dependency sequence](../README.md#dependency-sequence) and role-generation slice; renewable leases wait for the first-adopter gate. **Destination:** existing #1, #3, and #10 for v1; proposed P5 for post-v1 leases. **Cutover:** Yes for the v1 fence/role proof; no for deferred renewable leases.
...
- [GH-815 — Make claim_task preflight target the task's verified managed worktree](https://github.com/pixexid/llm-collab/issues/815) — **MIGRATE** — preflight must judge the task's exact verified environment and repository target, never a convenient canonical checkout. **Order:** after WorkItem/Assignment target binding and before activation/review. **Destination:** existing #7 and #13 plus the review/release and environment-safety slice; proposed P2 supplies the BB-native environment evidence. **Cutover:** Yes for any project whose activation/review uses managed worktrees.
- [GH-818 — Make post-merge cleanup report exact blockers separately from preserved worktrees](https://github.com/pixexid/llm-collab/issues/818) — **MIGRATE** — cleanup must distinguish exact destructive blockers from preserved evidence and never turn an empty loop into success. **Order:** after assignment/review evidence in the review/release and environment-safety slice. **Destination:** existing #13 and #22, with the founding ADR cleanup refusal contract; no second cleanup ledger. **Cutover:** Yes for a project using the cleanup gate.
- [GH-820 — Make claim_task release evidence select the task repository target](https://github.com/pixexid/llm-collab/issues/820) — **MIGRATE** — release evidence must bind to the task's exact registered repository target instead of a singular project fallback. **Order:** after #7 target projection and before release/closure. **Destination:** existing #7 and #22 plus the review/release and environment-safety slice. **Cutover:** Yes for multi-repository projects; otherwise the source remains the maintenance owner.
...
## Proposed bb-collab queue issues for missing BB-native work
...
Implement the migration/cutover gate for one project without copying llm-collab's
...
Finish adversarial conformance and the first-adopter lane as one bounded release gate for a quiet standard
...
`orchestrator:bb-collab` approver and its exact ten-class allowlist, including
