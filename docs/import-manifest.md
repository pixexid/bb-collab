...
| 5. Lane discipline | Amend | One writer per lane; contract v13 replaces the founding hard-2 project ceiling with the explicit per-orchestrator `extensions.bbCollab.writingLaneCeiling` dial, defaulting to 3 for bb-collab and lowerable through canonical config without a silent raise. Read-only work is assigned and isolated without consuming the cap. |
