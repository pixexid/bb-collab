...
| 5. Lane discipline | Amend | Contract v13 replaces the founding hard-2 project ceiling with the explicit per-orchestrator `extensions.bbCollab.writingLaneCeiling` dial, defaulting to 3 for bb-collab and lowerable through canonical config without a silent raise. Read-only work is assigned and isolated without consuming the cap. Contract v14 adds the director-seat requirement with zero writing-lane capacity; contract v15 permits only the exact current-generation unmanaged qualification record and makes no assignment/dispatch change. |
