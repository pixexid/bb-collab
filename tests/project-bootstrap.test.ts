...
const fixtureProjectIds = ["fixture_project_alpha_0001", "fixture_project_beta_0002"];
...
describe("GH-641 bootstrap defaults", () => {
  it("requires all watcher bindings, blind queue disposition, and rollback verification", () => {
    ...
    expect(bootstrap).toContain("stall-guard-graduation");
    expect(server).toContain("bootstrap_derivation_receipts");
    expect(server).toContain("readTenantTrust: (projectId) =>");
    expect(server).toContain("stallGuardTrustFromCanonical");
    expect(server).toContain("repository_targets");
  ...
  it("points watcher population at canonical inventory instead of a copied tenant list", () => {
    ...
    expect(server).toContain("githubRepositoryTarget");
    expect(server).toContain("if (rawMappings === undefined)");
    expect(server).toContain("return targetMappings.some((mapping) => mapping === null) ? null : targetMappings");
    expect(companion).toContain("native project inventory (`bb.sdk.projects.list()`)");
    for (const fixtureProjectId of fixtureProjectIds) expect(server).not.toContain(fixtureProjectId);
  });
});
