...
async function stallGuardArtifactFixture(initialLanes: readonly ("a" | "b")[] = ["a", "b"]) {
  const fixture = await assignmentFixture();
  const { host, db, fenceToken } = fixture;
  activateReviewer(db, fenceToken);
  const adapter = new DeterministicNativeAssignmentAdapter();
  const attempts = new Map<"a" | "b", string>();
  const addLane = (lane: "a" | "b") => {
    const assignmentId = `artifact-lane-${lane}`;
    const request = assignmentPrepareRequest(fenceToken, assignmentId);
    const prepared = applyWithFixtureReceipt(db, {
      ...request,
      assignment: { ...request.assignment!, laneId: assignmentId },
    }, null, null, adapter);
    expect(prepared.outcome).toBe("OK");
    const executionAttemptId = (prepared.evidence as { executionAttemptId: string }).executionAttemptId;
    db.prepare("UPDATE execution_attempts SET thread_id = ? WHERE execution_attempt_id = ?")
      .run(`${assignmentId}-thread`, executionAttemptId);
    attempts.set(lane, executionAttemptId);
  };
  for (const lane of initialLanes) {
    addLane(lane);
  }
  const observations = new Map<string, string | "unavailable">([
    ["environment-artifact-lane-a", "first"],
    ["environment-artifact-lane-b", "steady"],
  ]);
  const pullRequestStarts = new Map<"a" | "b", number>();
  let releaseLaneA: (() => void) | undefined;
  let laneAGate: Promise<void> | undefined;
  let holderStatus: "idle" | "active" = "idle";
  let holderReads = 0;
  let roleThreadReads = 0;
  let failRoleSteerReads = false;
  host.harness.sdk.stub("threads.get", (async ({ threadId }: { threadId: string }) => {
    const lane = threadId.startsWith("artifact-lane-");
    if (!lane) {
      holderReads += 1;
      roleThreadReads += 1;
      if (failRoleSteerReads && roleThreadReads % 2 === 0) throw new Error("role liveness unavailable");
    }
    return makeThreadResponse({
      id: threadId,
      projectId: PROJECT_ID,
      environmentId: lane ? `environment-${threadId.slice(0, -"-thread".length)}` : ROLE_ENVIRONMENT_ID,
      status: lane ? "idle" : holderStatus,
      archivedAt: null,
      deletedAt: null,
    });
  }) as never);
  host.harness.sdk.stub("threads.interactions.list", (async () => []) as never);
  host.harness.sdk.stub("threads.send", (async () => ({ ok: true })) as never);
  host.harness.sdk.stub("environments.pullRequest", (async ({ environmentId }: { environmentId: string }) => {
    const lane = environmentId.endsWith("lane-a") ? "a" : "b";
    pullRequestStarts.set(lane, (pullRequestStarts.get(lane) ?? 0) + 1);
    if (lane === "a") await laneAGate;
    const observation = observations.get(environmentId);
    if (observation === "unavailable") return { outcome: "unavailable", message: "fixture unavailable" };
    return {
      outcome: "available",
      pullRequest: {
        number: environmentId.endsWith("lane-a") ? 17 : 18,
        title: "Active lane",
        state: "open",
        url: `https://example.test/${environmentId}`,
        baseRefName: "main",
        headRefName: `bb/${environmentId}`,
        updatedAt: observation,
        checks: { state: "passing", totalCount: 1, passedCount: 1, failedCount: 0, pendingCount: 0 },
  ...
  return {
    host,
    setObservation(lane: "a" | "b", value: string | "unavailable") { observations.set(`environment-artifact-lane-${lane}`, value); },
    setHolderStatus(status: "idle" | "active") { holderStatus = status; },
    holderReads() { return holderReads; },
    failRoleSteerReads() { failRoleSteerReads = true; },
    roleThreadReads() { return roleThreadReads; },
    addLane,
    removeLane(lane: "a" | "b") { db.prepare("UPDATE execution_attempts SET state = 'failed' WHERE execution_attempt_id = ?").run(attempts.get(lane)); },
    setLaneState(lane: "a" | "b", state: string) { db.prepare("UPDATE execution_attempts SET state = ? WHERE execution_attempt_id = ?").run(state, attempts.get(lane)); },
    setWritingLaneCeiling(ceiling: number) {
      const row = db.prepare("SELECT canonical_config_json FROM project_config_revisions WHERE project_id = ? AND config_revision = 1").get(PROJECT_ID) as { canonical_config_json: string };
      const config = JSON.parse(row.canonical_config_json) as { extensions: { bbCollab: Record<string, unknown> } };
      config.extensions.bbCollab.writingLaneCeiling = ceiling;
      db.prepare("UPDATE project_config_revisions SET canonical_config_json = ? WHERE project_id = ? AND config_revision = 1").run(canonicalJson(config), PROJECT_ID);
    },
    holdLaneA() { laneAGate = new Promise((resolve) => { releaseLaneA = resolve; }); },
    releaseLaneA() { releaseLaneA?.(); laneAGate = undefined; releaseLaneA = undefined; },
    pullRequestStarts(lane: "a" | "b") { return pullRequestStarts.get(lane) ?? 0; },
    cycle: async () => JSON.parse((await host.harness.runCli(["stall-guard", "--cycle", "--project", PROJECT_ID])).stdout).evidence,
  };
}
...
describe("bb-collab plugin boundary", () => {
  ...
  it("stays silent for an active declared artifact-lane wait", async () => {
    ...
    try {
      const fixture = await fleetWatchdogFixture(0);
      await addPendingReview(fixture);
      fixture.db.prepare("UPDATE execution_attempts SET thread_id = ? WHERE origin = 'assignment' AND state = 'prepared'").run("waiting-artifact-lane");
      await fixture.host.harness.callRpc("registerWait", { waitId: "artifact-wait", waiterThreadId: "waiting-artifact-lane", sourceThreadId: "artifact-source", sourceEvent: "terminal", deadlineAtMs: 10 * 60 * 60_000, wakerSchedule: "stall-guard-liveness" });
  ...
  it("surfaces a stale declared wait to the orchestrator", async () => {
    ...
    try {
      const fixture = await fleetWatchdogFixture();
      await addPendingReview(fixture);
      fixture.db.prepare("UPDATE execution_attempts SET thread_id = ? WHERE origin = 'assignment' AND state = 'prepared'").run("stale-artifact-lane");
      await fixture.host.harness.callRpc("registerWait", { waitId: "stale-wait", waiterThreadId: "stale-artifact-lane", sourceThreadId: "artifact-source", sourceEvent: "terminal", deadlineAtMs: 48 * 60 * 60_000, wakerSchedule: "stall-guard-liveness" });
  ...
  it("continues processing other projects when one project's interactions read rejects", async () => {
    ...
    try {
      ...
      clock.mockReturnValue(2 * 60 * 60_000);
      await fixture.host.harness.runSchedule("fleet-watchdog");
      expect(projectOneInteractionReads).toBe(4);
  ...
  it("wakes for an active lane PR change even when the holder environment has no PR", async () => {
    const { host, db, fenceToken } = await assignmentFixture();
    const adapter = new DeterministicNativeAssignmentAdapter();
    const prepared = applyWithFixtureReceipt(db, assignmentPrepareRequest(fenceToken, "artifact-worker"), null, null, adapter);
    expect(prepared.outcome).toBe("OK");
    const executionAttemptId = (prepared.evidence as { executionAttemptId: string }).executionAttemptId;
    db.prepare("UPDATE execution_attempts SET thread_id = ? WHERE execution_attempt_id = ?").run("artifact-worker-thread", executionAttemptId);

    let workerUpdatedAt = "first";
    host.harness.sdk.stub("threads.get", (async ({ threadId }: { threadId: string }) => makeThreadResponse({
      id: threadId,
      projectId: PROJECT_ID,
      environmentId: threadId === "artifact-worker-thread" ? "environment-artifact-worker" : ROLE_ENVIRONMENT_ID,
      status: "idle",
      archivedAt: null,
      deletedAt: null,
    })) as never);
    ...
    host.harness.sdk.stub("threads.send", (async () => ({ ok: true })) as never);
    host.harness.sdk.stub("environments.pullRequest", (async ({ environmentId }: { environmentId: string }) => {
      if (environmentId === ROLE_ENVIRONMENT_ID) return { outcome: "absent" };
      return {
        outcome: "available",
        pullRequest: {
          number: 17,
          ...
          baseRefName: "main",
          headRefName: "bb/artifact-worker",
    ...
    const first = JSON.parse((await host.harness.runCli(["stall-guard", "--cycle", "--project", PROJECT_ID])).stdout);
    expect(first.evidence).toMatchObject({
      changed: 1,
      attempted: 0,
      steered: 0,
    });
    workerUpdatedAt = "changed";
    expect(JSON.parse((await host.harness.runCli(["stall-guard", "--cycle", "--project", PROJECT_ID])).stdout).evidence).toMatchObject({
      attempted: 1,
      verified: 1,
      steered: 1,
    });
    expect(host.harness.inspection.sdk.callsTo("threads.send")).toContainEqual([expect.objectContaining({ threadId: ROLE_THREAD_ID })]);
  });

  it("wakes for lane A while lane B is unavailable", async () => {
    const fixture = await stallGuardArtifactFixture();
    await fixture.cycle();
    fixture.setObservation("a", "changed");
    fixture.setObservation("b", "unavailable");

    const result = await fixture.cycle();

    expect(result).toMatchObject({ attempted: expect.any(Number), verified: expect.any(Number), steered: expect.any(Number) });
    expect(result.attempted).toBeGreaterThan(0);
    expect(result.verified).toBeGreaterThan(0);
    expect(result.steered).toBeGreaterThan(0);
  });

  it("does not wake when an unavailable lane recovers without an artifact change", async () => {
    const fixture = await stallGuardArtifactFixture();
    await fixture.cycle();
    const sends = fixture.host.harness.inspection.sdk.callsTo("threads.send").length;
    fixture.setObservation("b", "unavailable");
    expect(await fixture.cycle()).toMatchObject({ attempted: 0, verified: 0, steered: 0 });
    fixture.setObservation("b", "steady");

    expect(await fixture.cycle()).toMatchObject({ attempted: 0, verified: 0, steered: 0 });
    expect(fixture.host.harness.inspection.sdk.callsTo("threads.send")).toHaveLength(sends);
  });

  it("does not re-evaluate an artifact delta refused for an active holder", async () => {
    const fixture = await stallGuardArtifactFixture();
    await fixture.cycle();
    fixture.setHolderStatus("active");
    fixture.setObservation("a", "changed");
    const before = fixture.holderReads();

    expect(await fixture.cycle()).toMatchObject({ attempted: 0, verified: 0, steered: 0 });
    const afterRefusal = fixture.holderReads();
    await fixture.cycle();

    expect(afterRefusal - before).toBe(4);
    expect(fixture.holderReads() - afterRefusal).toBe(2);
  });

  it("retries an artifact delta when the final role liveness read fails", async () => {
    const fixture = await stallGuardArtifactFixture();
    await fixture.cycle();
    const before = fixture.roleThreadReads();
    fixture.failRoleSteerReads();
    fixture.setObservation("a", "changed");

    expect(await fixture.cycle()).toMatchObject({ changed: 0, attempted: 0, verified: 0, steered: 0 });
    const afterFirst = fixture.roleThreadReads();
    expect(await fixture.cycle()).toMatchObject({ changed: 0, attempted: 0, verified: 0, steered: 0 });
    expect(afterFirst - before).toBeGreaterThan(0);
    expect(fixture.roleThreadReads() - afterFirst).toBeGreaterThan(0);
  });

  it("wakes when a newly observed lane has an available PR", async () => {
    const fixture = await stallGuardArtifactFixture(["a"]);
    await fixture.cycle();
    fixture.addLane("b");
  ...
  it("wakes when a terminal writer frees capacity for an unchanged queued lane", async () => {
    const fixture = await stallGuardArtifactFixture(["a", "b"]);
    fixture.setWritingLaneCeiling(1);
    fixture.setLaneState("a", "running");
    await fixture.cycle();
    fixture.setLaneState("a", "done");
  ...
  it("does not wake when a lane disappears without a queued successor", async () => {
    const fixture = await stallGuardArtifactFixture(["a"]);
    fixture.setLaneState("a", "running");
    await fixture.cycle();
    fixture.removeLane("a");

    expect(await fixture.cycle()).toMatchObject({ changed: 2, attempted: 0, verified: 0, steered: 0 });
  });
  ...
  it("reads each project lane once per cycle regardless of holder count", async () => {
    const fixture = await stallGuardArtifactFixture();

    await fixture.cycle();

    const laneReads = fixture.host.harness.inspection.sdk.callsTo("threads.get")
      .filter(([input]) => (input as { threadId: string }).threadId.startsWith("artifact-lane-"));
    expect(laneReads).toHaveLength(4);
    expect(fixture.host.harness.inspection.sdk.callsTo("environments.pullRequest")).toHaveLength(2);
  });

  it("does not start lane B's artifact read before lane A settles", async () => {
    const fixture = await stallGuardArtifactFixture();
    fixture.holdLaneA();
    const cycle = fixture.cycle();

    await vi.waitFor(() => expect(fixture.pullRequestStarts("a")).toBe(1));
    expect(fixture.pullRequestStarts("b")).toBe(0);
    fixture.releaseLaneA();
    await cycle;
    expect(fixture.pullRequestStarts("b")).toBe(1);
  });
  ...
  it("appends the v21 visible-configuration contract and rolls every cached consumer forward", () => {
    ...
    expect(cachedConsumerRolloutEvidence(cachedConsumerObservations(12, 21))).toMatchObject({
      oldSchemaVersion: 11,
      newSchemaVersion: 12,
      oldContractVersion: 20,
      newContractVersion: 21,
  ...
  it("assembles the production v21 cached-consumer rollout receipt with stale-v20 refusal semantics", async () => {
    ...
    expect(JSON.parse(evidence.durableRefJson)).toMatchObject({
      reread: { observations: CACHED_CONSUMERS.map((name) => ({ name, observedSchemaVersion: 12, observedContractVersion: 21 })), action: "reread", expected: 4, attempted: 4, verified: 4 },
  ...
  it("runs the closed lifecycle with exact replay/CAS, canonical export/import, and target-safe retirement", async () => {
    ...
    expect(firstExport.manifest).toMatchObject({ schemaVersion: 12, schemaDigest, contractVersion: 21, contractDigest });
  ...
  it("migrates v3 role evidence to v4 without manufacturing a canonical holder attempt", async () => {
    ...
    expect(cachedConsumerRolloutEvidence(cachedConsumerObservations(11, 19))).toMatchObject({ oldSchemaVersion: 11, newSchemaVersion: 12, oldContractVersion: 20, newContractVersion: 21, action: "refused", expected: 4, attempted: 4, verified: 0 });
    expect(cachedConsumerRolloutEvidence(cachedConsumerObservations(12, 19))).toMatchObject({ oldSchemaVersion: 11, newSchemaVersion: 12, oldContractVersion: 20, newContractVersion: 21, action: "refused", expected: 4, attempted: 4, verified: 0 });
    expect(cachedConsumerRolloutEvidence(cachedConsumerObservations(12, 20))).toMatchObject({ oldSchemaVersion: 11, newSchemaVersion: 12, oldContractVersion: 20, newContractVersion: 21, action: "refused", expected: 4, attempted: 4, verified: 0 });
    expect(cachedConsumerRolloutEvidence(cachedConsumerObservations(12, 21))).toMatchObject({ oldSchemaVersion: 11, newSchemaVersion: 12, oldContractVersion: 20, newContractVersion: 21, action: "reread", expected: 4, attempted: 4, verified: 4 });
  });
