...
import {
  ...
  GH644_LOCAL_REVIEW_AUTHORITY_MIGRATION_ID,
...
function dispatchSpawn(parentThreadId: string, overrides: Record<string, unknown> = {}) {
  return {
    projectId: PROJECT_ID,
    parentThreadId,
    environment: { type: "host" as const, hostId: "host-main", workspace: { type: "managed-worktree" as const, baseBranch: { kind: "default" as const } } },
    title: "lane",
    prompt: "lane brief",
    ...ROLE_PROFILE,
    executionInputSources: EXPLICIT_EXECUTION_INPUT_SOURCES,
    ...overrides,
  };
}
...
function appendEquivalentConfigRevision(db: Database.Database, fenceToken: string, note: string) {
  ...
  const config = JSON.parse(stored.canonical_config_json) as Record<string, unknown>;
  (config.extensions as Record<string, unknown>).bbCollab = {
    ...((config.extensions as Record<string, unknown>).bbCollab as Record<string, unknown>),
    cutoverNote: note,
  };
  const target = db.prepare(
    "SELECT repo_target_id AS repoTargetId, source_id AS sourceId, host_id AS hostId, path, remote_url AS remoteUrl, default_branch AS defaultBranch FROM repository_targets WHERE project_id = ? AND config_revision = 1",
  ).get(PROJECT_ID) as { repoTargetId: string; sourceId: string; hostId: string; path: string; remoteUrl: string | null; defaultBranch: string };
  return applyWithFixtureReceipt(db, {
    ...bootstrapRequest(),
    operationClass: "config_revision",
    idempotencyKey: `dispatch-equivalent-${note}`,
    expectedConfigRevision: 1,
    configRevision: 2,
    expectedGovernanceEpoch: 1,
    expectedFenceToken: fenceToken,
...
`);
  ...
  it("registers a second synthetic project through the strict seam and preserves tenant isolation", async () => {
    const host = await loadedHost();
    const db = host.bb.storage.database();
    const sourceActor = RECEIPT_ID;
    seedVerifiedFixtureReceipt(db, { projectId: PROJECT_ID, receiptId: sourceActor, actorKind: "plugin", subjectId: PLUGIN_ID });
    const sourceBootstrap = applyFixtureMutation(db, bootstrapRequest(PROJECT_ID, { config: roleConfig(), actorReceiptId: sourceActor }));
    expect(sourceBootstrap).toMatchObject({ outcome: "OK" });
    const sourceBootstrapFence = (sourceBootstrap.evidence as { fenceToken: string }).fenceToken;
    const sourceClaim = applyFixtureMutation(db, {
      projectId: PROJECT_ID,
      operationClass: "governor_claim",
      idempotencyKey: "claim-source-tenant",
      actorReceiptId: sourceActor,
      expectedConfigRevision: 1,
      configRevision: 1,
      expectedGovernanceEpoch: 1,
      expectedFenceToken: sourceBootstrapFence,
      repoTargetId: null,
    });
    expect(sourceClaim).toMatchObject({ outcome: "OK", currentGovernanceEpoch: 2 });
    const sourceFence = (sourceClaim.evidence as { fenceToken: string }).fenceToken;
    const sourceQualification = applyFixtureMutation(db, qualificationRequest(sourceFence, {
      actorReceiptId: sourceActor,
      idempotencyKey: "source-orchestrator-qualification",
      qualificationId: "source-orchestrator-qualification",
      expectedGovernanceEpoch: 2,
      expectedFenceToken: sourceFence,
    }), null, roleReader());
    expect(sourceQualification).toMatchObject({ outcome: "OK" });
    const sourceSuccession = applyFixtureMutation(db, successionRequest(sourceFence, {
      actorReceiptId: sourceActor,
      idempotencyKey: "source-orchestrator-succession",
      qualificationId: "source-orchestrator-qualification",
      expectedGovernanceEpoch: 2,
      expectedFenceToken: sourceFence,
    }), null, roleReader());
    expect(sourceSuccession).toMatchObject({ outcome: "OK" });
    const sourceRoleActor = "source-orchestrator-actor";
    seedVerifiedFixtureReceipt(db, {
      projectId: PROJECT_ID,
      receiptId: sourceRoleActor,
      actorKind: "role",
      subjectId: (sourceSuccession.evidence as { holderExecutionAttemptId: string }).holderExecutionAttemptId,
      roleId: "project-orchestrator",
      roleGeneration: 1,
    });
    const secondProject = "proj_second_tenant";
    const adoptSourceDecision = (decisionId: string, decision: NonNullable<ApplyRequest["decision"]>) => {
      const scope = decision.scope as { operation?: string; sourceProjectId?: string; targetProjectId?: string; repoTargetId?: null };
      const options = decision.options as { rootOfTrust?: string };
      const actorReceiptId = decision.decisionClass === "operator_only" && scope.operation === "cross_project_bootstrap" &&
        scope.sourceProjectId === PROJECT_ID && scope.targetProjectId !== PROJECT_ID && scope.repoTargetId === null &&
        options.rootOfTrust === "host_local_operator" ? sourceActor : sourceRoleActor;
      expect(applyFixtureMutation(db, decisionCreateRequest(sourceFence, decisionId, {
        actorReceiptId,
        expectedGovernanceEpoch: 2,
        expectedFenceToken: sourceFence,
        repoTargetId: null,
      ...
      expect(applyFixtureMutation(db, decisionDispositionRequest(sourceFence, decisionId, 1, {
        actorReceiptId,
        expectedGovernanceEpoch: 2,
        expectedFenceToken: sourceFence,
        repoTargetId: null,
        idempotencyKey: `adopt-${decisionId}`,
      }))).toMatchObject({ outcome: "OK" });
    };
    const bootstrapDecision = (decisionId: string, overrides: Partial<NonNullable<ApplyRequest["decision"]>> = {}): NonNullable<ApplyRequest["decision"]> => ({
      decisionId,
      repoTargetId: null,
      scope: { operation: "cross_project_bootstrap", sourceProjectId: PROJECT_ID, targetProjectId: secondProject, repoTargetId: null },
      decisionClass: "operator_only",
      options: { rootOfTrust: "host_local_operator" },
      resourceRevision: 1,
      ...overrides,
    });
    adoptSourceDecision("source-unrelated-class", bootstrapDecision("source-unrelated-class", { decisionClass: "assignment_admission" }));
    adoptSourceDecision("source-wrong-target", bootstrapDecision("source-wrong-target", {
      scope: { operation: "cross_project_bootstrap", sourceProjectId: PROJECT_ID, targetProjectId: "proj_wrong_target", repoTargetId: null },
    }));
    adoptSourceDecision("source-wrong-source", bootstrapDecision("source-wrong-source", {
      scope: { operation: "cross_project_bootstrap", sourceProjectId: "proj_wrong_source", targetProjectId: secondProject, repoTargetId: null },
    }));
    adoptSourceDecision("source-wrong-scope", bootstrapDecision("source-wrong-scope", {
      scope: { operation: "not_cross_project_bootstrap", sourceProjectId: PROJECT_ID, targetProjectId: secondProject, repoTargetId: null },
    }));
    adoptSourceDecision("source-wrong-options", bootstrapDecision("source-wrong-options", { options: { rootOfTrust: "untrusted" } }));
    adoptSourceDecision("source-bootstrap-decision", bootstrapDecision("source-bootstrap-decision"));
    const otherProject = "proj_other_tenant";
    adoptSourceDecision("source-other-bootstrap-decision", bootstrapDecision("source-other-bootstrap-decision", {
      scope: { operation: "cross_project_bootstrap", sourceProjectId: PROJECT_ID, targetProjectId: otherProject, repoTargetId: null },
    }));
    const existingBefore = exportFoundation(db, PROJECT_ID);

    const secondActor = "second-tenant-genesis";
    const input = {
      ...
      bootstrapAuthority: {
        derivationId: "derive-second-tenant",
        genesisReceiptId: secondActor,
        sourceProjectId: PROJECT_ID,
        sourceGovernanceEpoch: 2,
        sourceFenceToken: sourceFence,
        authorizingDecisionId: "source-bootstrap-decision",
        authorizingDispositionSequence: 1,
      },
    };
    for (const [suffix, decisionId] of [
      ["unrelated-class", "source-unrelated-class"],
      ["wrong-target", "source-wrong-target"],
      ["wrong-source", "source-wrong-source"],
      ["wrong-scope", "source-wrong-scope"],
      ["wrong-options", "source-wrong-options"],
    ] as const) {
      const invalidAuthority = await host.harness.callRpc("registerProject", {
        ...input,
        idempotencyKey: `register-${suffix}-authority`,
        bootstrapAuthority: {
          ...input.bootstrapAuthority,
          derivationId: `derive-${suffix}-authority`,
          genesisReceiptId: `genesis-${suffix}-authority`,
    ...
    const unadopted = await host.harness.callRpc("registerProject", {
      ...
      bootstrapAuthority: { ...input.bootstrapAuthority, authorizingDispositionSequence: 99 },
    }) as FoundationResult;
    expect(unadopted).toMatchObject({ outcome: "DECISION_DISPOSITION_INVALID", attempted: 0, verified: 0 });
    const nonPluginSource = "proj_non_plugin_source";
    const nonPluginActor = "non-plugin-source-actor";
    seedVerifiedFixtureReceipt(db, { projectId: nonPluginSource, receiptId: nonPluginActor });
    const nonPluginBootstrap = applyFixtureMutation(db, bootstrapRequest(nonPluginSource, { actorReceiptId: nonPluginActor, config: roleConfig() }));
    expect(nonPluginBootstrap).toMatchObject({ outcome: "OK" });
    const nonPlugin = await host.harness.callRpc("registerProject", {
      ...input,
      projectId: "proj_non_plugin_target",
      idempotencyKey: "register-non-plugin-source",
      bootstrapAuthority: {
        ...input.bootstrapAuthority,
        derivationId: "derive-non-plugin-target",
        genesisReceiptId: "non-plugin-target-genesis",
        sourceProjectId: nonPluginSource,
        sourceGovernanceEpoch: 1,
        sourceFenceToken: (nonPluginBootstrap.evidence as { fenceToken: string }).fenceToken,
      },
    }) as FoundationResult;
    expect(nonPlugin).toMatchObject({ outcome: "BOOTSTRAP_SOURCE_INVALID", attempted: 0, verified: 0 });
    const callerReceipt = await host.harness.runCli([
      "register-project", "--project", secondProject, "--request", JSON.stringify({
        ...input,
        idempotencyKey: "register-caller-receipt",
        actorReceiptId: "foreign-target-receipt",
      }),
    ]);
    ...
    expect(registered).toMatchObject({
      ...
      evidence: { bootstrapDerivation: { sourceProjectId: PROJECT_ID, authorizingDecisionId: "source-bootstrap-decision" } },
    });
    ...
    const operationalActor = (registered.evidence as { bootstrapDerivation: { operationalActorReceiptId: string } }).bootstrapDerivation.operationalActorReceiptId;
    expect(db.prepare("SELECT project_id, genesis_receipt_id, operational_actor_receipt_id, source_project_id, source_governance_epoch, authorizing_decision_id, authorizing_disposition_sequence FROM bootstrap_derivation_receipts WHERE project_id = ?").get(secondProject)).toEqual({
      project_id: secondProject,
      genesis_receipt_id: secondActor,
      operational_actor_receipt_id: operationalActor,
      source_project_id: PROJECT_ID,
      source_governance_epoch: 2,
      authorizing_decision_id: "source-bootstrap-decision",
      authorizing_disposition_sequence: 1,
    });
    expect(db.prepare("SELECT actor_kind, subject_id, verification_state FROM actor_receipts WHERE project_id = ? AND receipt_id = ?").get(secondProject, secondActor)).toEqual({ actor_kind: "plugin", subject_id: PLUGIN_ID, verification_state: "verified" });
    expect(db.prepare("SELECT actor_kind, subject_id, verification_state FROM actor_receipts WHERE project_id = ? AND receipt_id = ?").get(secondProject, operationalActor)).toEqual({ actor_kind: "plugin", subject_id: PLUGIN_ID, verification_state: "verified" });
    expect(db.prepare("SELECT actor_receipt_id FROM project_governorships WHERE project_id = ? AND governance_epoch = 1").get(secondProject)).toEqual({ actor_receipt_id: operationalActor });
    const beforeMalformed = exportFoundation(db, secondProject);
    const otherRegistered = await host.harness.callRpc("registerProject", {
      ...input,
      projectId: otherProject,
      idempotencyKey: "register-other-tenant",
      bootstrapAuthority: {
        ...input.bootstrapAuthority,
        derivationId: "derive-other-tenant",
        genesisReceiptId: "other-tenant-genesis",
        authorizingDecisionId: "source-other-bootstrap-decision",
      },
    }) as FoundationResult;
    expect(otherRegistered).toMatchObject({ outcome: "OK", subject: otherProject });
    ...
    const reused = await host.harness.callRpc("registerProject", {
      ...input,
      projectId: "proj_retargeted_tenant",
      idempotencyKey: "register-reused-genesis",
    }) as FoundationResult;
    expect(reused).toMatchObject({ outcome: "BOOTSTRAP_DERIVATION_REUSED", attempted: 0, verified: 0 });
    const retargeted = await host.harness.callRpc("registerProject", {
      ...input,
      projectId: "proj_retargeted_tenant",
      idempotencyKey: "register-retargeted-genesis",
      bootstrapAuthority: { ...input.bootstrapAuthority, derivationId: "derive-retargeted-tenant" },
    }) as FoundationResult;
    expect(retargeted).toMatchObject({ outcome: "BOOTSTRAP_DERIVATION_REUSED", attempted: 0, verified: 0 });
    ...
    expect(applyFixtureMutation(db, {
      projectId: secondProject,
      operationClass: "governor_claim",
      idempotencyKey: "genesis-reuse-governor",
    ...
    const storedSecondConfig = db.prepare(
      "SELECT canonical_config_json FROM project_config_revisions WHERE project_id = ? AND config_revision = 1",
    ).get(secondProject) as { canonical_config_json: string };
    expect(applyFixtureMutation(db, {
      projectId: secondProject,
      operationClass: "config_revision",
      idempotencyKey: "genesis-reuse-config",
      actorReceiptId: secondActor,
      expectedConfigRevision: 1,
      configRevision: 2,
      expectedGovernanceEpoch: 1,
      expectedFenceToken: registerEvidence.fenceToken,
      repoTargetId: null,
      config: JSON.parse(storedSecondConfig.canonical_config_json),
      targets: bootstrapRequest().targets,
    })).toMatchObject({ outcome: "BOOTSTRAP_GENESIS_REUSED", attempted: 0, verified: 0 });
    expect(applyFixtureMutation(db, decisionCreateRequest(registerEvidence.fenceToken, "genesis-reuse-decision", {
      projectId: secondProject,
      actorReceiptId: secondActor,
      expectedGovernanceEpoch: 1,
      expectedFenceToken: registerEvidence.fenceToken,
      repoTargetId: null,
      idempotencyKey: "genesis-reuse-decision-create",
      decision: {
        decisionId: "genesis-reuse-decision",
        repoTargetId: null,
        scope: { operation: "genesis-reuse" },
        decisionClass: "assignment_admission",
        options: { mode: "exact" },
        resourceRevision: 1,
      },
    }))).toMatchObject({ outcome: "BOOTSTRAP_GENESIS_REUSED", attempted: 0, verified: 0 });
    expect(applyFixtureMutation(db, qualificationRequest(registerEvidence.fenceToken, {
      projectId: secondProject,
      idempotencyKey: "genesis-reuse-qualification",
      actorReceiptId: secondActor,
      expectedGovernanceEpoch: 1,
      expectedFenceToken: registerEvidence.fenceToken,
    }), null, roleReader((facts) => {
      facts.thread.projectId = secondProject;
      facts.environment.projectId = secondProject;
      facts.project = projectFacts(secondProject);
    }))).toMatchObject({ outcome: "BOOTSTRAP_GENESIS_REUSED", attempted: 0, verified: 0 });
    expect(applyFixtureMutation(db, successionRequest(registerEvidence.fenceToken, {
      projectId: secondProject,
      idempotencyKey: "genesis-reuse-succession",
      actorReceiptId: secondActor,
      expectedGovernanceEpoch: 1,
      expectedFenceToken: registerEvidence.fenceToken,
    }), null, roleReader((facts) => {
    ...
    const claimed = applyFixtureMutation(db, {
      ...
      actorReceiptId: operationalActor,
      expectedConfigRevision: 1,
      configRevision: 1,
      expectedGovernanceEpoch: 1,
      expectedFenceToken: registerEvidence.fenceToken,
  ...
  it("archives an inbox message and excludes it from the default reader", async () => {
    const clock = vi.spyOn(Date, "now").mockReturnValue(1_100_000);
    try {
      const host = hostFor();
      await plugin(host.bb, { notifyUrgent: async () => undefined });
      const { db } = seedAndBootstrap(host);
...
`);
  ...
  it("proves equivalent config cutover before intent and records one governed continuation", async () => {
    ...
    (config.extensions as Record<string, unknown>).bbCollab = {
      ...((config.extensions as Record<string, unknown>).bbCollab as Record<string, unknown>),
      cutoverNote: "unrelated-director-seat-addition",
    };
    const target = fixture.db.prepare(
      "SELECT repo_target_id AS repoTargetId, source_id AS sourceId, host_id AS hostId, path, remote_url AS remoteUrl, default_branch AS defaultBranch FROM repository_targets WHERE project_id = ? AND config_revision = 1",
    ).get(PROJECT_ID) as { repoTargetId: string; sourceId: string; hostId: string; path: string; remoteUrl: string | null; defaultBranch: string };
    expect(applyWithFixtureReceipt(fixture.db, {
      ...bootstrapRequest(),
      operationClass: "config_revision",
      idempotencyKey: "dispatch-equivalent-config-2",
      expectedConfigRevision: 1,
      configRevision: 2,
      expectedGovernanceEpoch: 1,
  ...
  it("refuses a changed dispatch target before the durable intent or native spawn", async () => {
    ...
    const changedTarget = { ...target, path: "/workspace/changed" };
    expect(applyWithFixtureReceipt(fixture.db, {
      ...bootstrapRequest(),
      operationClass: "config_revision",
      idempotencyKey: "dispatch-changed-target-config-2",
      expectedConfigRevision: 1,
      configRevision: 2,
      expectedGovernanceEpoch: 1,
      expectedFenceToken: fixture.fenceToken,
      config: JSON.parse(stored.canonical_config_json),
  ...
  it.each([
    ...
    ["personal", { type: "host", workspace: { type: "personal" } }],
    ...
    const result = JSON.parse(await fixture.host.harness.callAgentTool("dispatch_lane", {
      request: transitionRequest(fixture.fenceToken, "in_progress", 2),
      spawn: dispatchSpawn(fixture.orchestratorThreadId, { environment }),
    }, { projectId: PROJECT_ID, threadId: fixture.orchestratorThreadId }) as string);
    expect(result.outcome).toBe("REPO_TARGET_FOREIGN");
    expect(fixture.host.harness.inspection.sdk.callsTo("threads.spawn")).toHaveLength(0);
    expect(exportFoundation(fixture.db, PROJECT_ID)).toEqual(before);
  });

  it("refuses partial, blind, or changed project source facts before dispatch intent", async () => {
    for (const mutate of [
      () => ({ id: PROJECT_ID, sources: [] }),
      () => { throw new Error("project facts unavailable"); },
      () => ({ ...projectFacts(), sources: [{ ...projectFacts().sources[0]!, path: "/workspace/foreign" }] }),
    ]) {
      const fixture = await fleetWatchdogFixture(0, true, 1, false);
      expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
      fixture.host.harness.sdk.stub("projects.get", (async () => mutate()) as never);
      const before = exportFoundation(fixture.db, PROJECT_ID);
      const result = JSON.parse(await fixture.host.harness.callAgentTool("dispatch_lane", {
        request: transitionRequest(fixture.fenceToken, "in_progress", 2),
        spawn: dispatchSpawn(fixture.orchestratorThreadId),
  ...
  it("binds a child when config advances during initial spawn", async () => {
    const fixture = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    ...
    fixture.host.harness.sdk.stub("threads.spawn", (async (input: { projectId: string; parentThreadId?: string; title?: string }) => {
      spawnCalls += 1;
      expect(appendEquivalentConfigRevision(fixture.db, fixture.fenceToken, "advance-initial-spawn")).toMatchObject({ outcome: "OK" });
      return makeThreadResponse({ id: "advance-initial-lane", projectId: input.projectId, parentThreadId: input.parentThreadId ?? null, title: input.title ?? null });
    }) as never);
    ...
    expect(fixture.db.prepare("SELECT state, thread_id FROM execution_attempts WHERE origin = 'work_item'").get()).toEqual({ state: "running", thread_id: "advance-initial-lane" });
    expect(fixture.db.prepare("SELECT event_json FROM state_events WHERE project_id = ? AND event_type = 'work_item_attempt_armed'").get(PROJECT_ID)).toMatchObject({ event_json: expect.stringContaining("bound_to_durable_intent") });
  });

  it("binds a delayed child when config advances during retry spawn", async () => {
    const fixture = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    const request = transitionRequest(fixture.fenceToken, "in_progress", 2);
    const spawn = dispatchSpawn(fixture.orchestratorThreadId);
    ...
    fixture.host.harness.sdk.stub("threads.spawn", (async (input: { projectId: string; parentThreadId?: string; title?: string }) => {
      ...
      expect(appendEquivalentConfigRevision(fixture.db, fixture.fenceToken, "advance-retry-spawn")).toMatchObject({ outcome: "OK" });
      return makeThreadResponse({ id: "advance-retry-lane", projectId: input.projectId, parentThreadId: input.parentThreadId ?? null, title: input.title ?? null });
    }) as never);
    const result = JSON.parse(await fixture.host.harness.callAgentTool("dispatch_lane", { request, spawn }, { projectId: PROJECT_ID, threadId: fixture.orchestratorThreadId }) as string);
    expect(result.outcome).toBe("OK");
    expect(spawnCalls).toBe(2);
    expect(fixture.db.prepare("SELECT state, thread_id FROM execution_attempts WHERE origin = 'work_item'").get()).toEqual({ state: "running", thread_id: "advance-retry-lane" });
  });

  it("binds a delayed successful child after a config advance and thrown spawn response", async () => {
    const fixture = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    const request = transitionRequest(fixture.fenceToken, "in_progress", 2);
    const spawn = dispatchSpawn(fixture.orchestratorThreadId);
    const child = makeThreadResponse({ id: "advance-delayed-lane", projectId: PROJECT_ID, parentThreadId: fixture.orchestratorThreadId, title: `${spawn.title} [dispatch:${request.idempotencyKey}]` });
    fixture.host.harness.sdk.stub("threads.spawn", (async () => {
      expect(appendEquivalentConfigRevision(fixture.db, fixture.fenceToken, "advance-delayed-response")).toMatchObject({ outcome: "OK" });
      throw new Error("response lost after child creation");
    }) as never);
    fixture.host.harness.sdk.stub("threads.list", (async ({ archived }: { archived?: boolean }) => archived ? [] : [child]) as never);
    const result = JSON.parse(await fixture.host.harness.callAgentTool("dispatch_lane", { request, spawn }, { projectId: PROJECT_ID, threadId: fixture.orchestratorThreadId }) as string);
    expect(result.outcome).toBe("OK");
    expect(fixture.host.harness.inspection.sdk.callsTo("threads.spawn")).toHaveLength(1);
    expect(fixture.db.prepare("SELECT state, thread_id FROM execution_attempts WHERE origin = 'work_item'").get()).toEqual({ state: "running", thread_id: "advance-delayed-lane" });
  });

  it("binds equivalent config cutover for the maintained-body dispatch shape", async () => {
    ...
    try {
      ...
      projected.fixture.host.harness.sdk.stub("threads.spawn", (async (input: { projectId: string; parentThreadId?: string; title?: string }) => {
        expect(appendEquivalentConfigRevision(projected.fixture.db, projected.fixture.fenceToken, "advance-maintained-body")).toMatchObject({ outcome: "OK" });
        return makeThreadResponse({ id: "maintained-body-advance-lane", projectId: input.projectId, parentThreadId: input.parentThreadId ?? null, title: input.title ?? null });
      }) as never);
      const result = JSON.parse(await projected.fixture.host.harness.callAgentTool("dispatch_lane", { request, spawn }, { projectId: PROJECT_ID, threadId: projected.fixture.orchestratorThreadId }) as string);
      expect(result.outcome).toBe("OK");
      expect(projected.fixture.db.prepare("SELECT state, thread_id FROM execution_attempts WHERE origin = 'work_item'").get()).toEqual({ state: "running", thread_id: "maintained-body-advance-lane" });
    } finally {
      restore.cleanup();
    }
  });
  ...
  it("rejects a managed-worktree spawn without hostId before canonical intent", async () => {
    const fixture = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    const result = JSON.parse(await fixture.host.harness.callAgentTool("dispatch_lane", {
      request: transitionRequest(fixture.fenceToken, "in_progress", 2),
    ...
    expect(result.message).toContain("hostId");
    expect(fixture.host.harness.inspection.sdk.callsTo("threads.spawn")).toHaveLength(0);
    expect(fixture.db.prepare("SELECT COUNT(*) AS count FROM execution_attempts WHERE origin = 'work_item'").get()).toEqual({ count: 0 });
    expect(fixture.db.prepare("SELECT lifecycle_state FROM work_items WHERE project_id = ? AND work_item_id = ?").get(PROJECT_ID, WORK_ITEM_ID)).toEqual({ lifecycle_state: "ready" });
  });

  it("rejects malformed structured prompt input before canonical intent", async () => {
    const fixture = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    const result = JSON.parse(await fixture.host.harness.callAgentTool("dispatch_lane", {
      request: transitionRequest(fixture.fenceToken, "in_progress", 2),
  ...
  it("finalizes one exact same-id dispatch thread after a lost spawn response", async () => {
    const fixture = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    const request = transitionRequest(fixture.fenceToken, "in_progress", 2);
    const spawn = dispatchSpawn(fixture.orchestratorThreadId);
    let spawnCalls = 0;
    fixture.host.harness.sdk.stub("threads.spawn", (async () => { spawnCalls += 1; throw new Error("response lost after create"); }) as never);
    ...
    expect(result.outcome).toBe("OK");
    expect(spawnCalls).toBe(1);
    expect(fixture.db.prepare("SELECT state, thread_id FROM execution_attempts WHERE origin = 'work_item'").get()).toEqual({ state: "running", thread_id: "recovered-lane" });
  });

  it("refuses an archived exact thread without retrying a possible created side effect", async () => {
    const fixture = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    const request = transitionRequest(fixture.fenceToken, "in_progress", 2);
    const spawn = dispatchSpawn(fixture.orchestratorThreadId);
  ...
  it("refuses active and archived duplicate dispatch identities as multiple evidence", async () => {
    const fixture = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    const request = transitionRequest(fixture.fenceToken, "in_progress", 2);
    const spawn = dispatchSpawn(fixture.orchestratorThreadId);
    let spawnCalls = 0;
    fixture.host.harness.sdk.stub("threads.spawn", (async () => { spawnCalls += 1; throw new Error("response lost after create"); }) as never);
    fixture.host.harness.sdk.stub("threads.list", (async ({ archived }: { archived?: boolean }) => [makeThreadResponse({
      id: archived ? "archived-lane" : "active-lane",
      projectId: PROJECT_ID,
      parentThreadId: fixture.orchestratorThreadId,
      title: `lane [dispatch:${request.idempotencyKey}]`,
  ...
  it("rejects same-marker evidence whose title changed from the recorded dispatch", async () => {
    const fixture = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    const request = transitionRequest(fixture.fenceToken, "in_progress", 2);
    const spawn = dispatchSpawn(fixture.orchestratorThreadId);
    ...
    fixture.host.harness.sdk.stub("threads.spawn", (async () => { spawnCalls += 1; throw new Error("response lost after create"); }) as never);
    fixture.host.harness.sdk.stub("threads.list", (async ({ archived }: { archived?: boolean }) => archived ? [] : [makeThreadResponse({
      id: "changed-title-lane",
      projectId: PROJECT_ID,
      parentThreadId: fixture.orchestratorThreadId,
      title: `changed [dispatch:${request.idempotencyKey}]`,
    })]) as never);
    const result = JSON.parse(await fixture.host.harness.callAgentTool("dispatch_lane", { request, spawn }, { projectId: PROJECT_ID, threadId: fixture.orchestratorThreadId }) as string);
    expect(result.outcome).toBe("EXTERNAL_DELIVERY_AMBIGUOUS");
    expect(spawnCalls).toBe(1);
  ...
  it("retries once after a complete no-match inventory and never retries blind evidence", async () => {
    const fixture = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    const request = transitionRequest(fixture.fenceToken, "in_progress", 2);
    const spawn = dispatchSpawn(fixture.orchestratorThreadId);
    ...
    fixture.host.harness.sdk.stub("threads.spawn", (async (input: { title?: string }) => {
      spawnCalls += 1;
      if (spawnCalls === 1) throw new Error("native pre-create refusal");
      return makeThreadResponse({ id: "retried-lane", projectId: PROJECT_ID, parentThreadId: fixture.orchestratorThreadId, title: input.title ?? null });
    }) as never);
    const result = JSON.parse(await fixture.host.harness.callAgentTool("dispatch_lane", { request, spawn }, { projectId: PROJECT_ID, threadId: fixture.orchestratorThreadId }) as string);
    expect(result.outcome).toBe("OK");
    expect(spawnCalls).toBe(2);
    expect(fixture.db.prepare("SELECT state, thread_id FROM execution_attempts WHERE origin = 'work_item'").get()).toEqual({ state: "running", thread_id: "retried-lane" });

    const blind = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(blind.db, transitionRequest(blind.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    const blindRequest = transitionRequest(blind.fenceToken, "in_progress", 2);
    const blindSpawn = dispatchSpawn(blind.orchestratorThreadId);
    let blindCalls = 0;
    blind.host.harness.sdk.stub("threads.list", (async () => { throw new Error("inventory unavailable"); }) as never);
    blind.host.harness.sdk.stub("threads.spawn", (async () => { blindCalls += 1; throw new Error("native refusal"); }) as never);
    const blindResult = JSON.parse(await blind.host.harness.callAgentTool("dispatch_lane", { request: blindRequest, spawn: blindSpawn }, { projectId: PROJECT_ID, threadId: blind.orchestratorThreadId }) as string);
    expect(blindResult.outcome).toBe("EXTERNAL_DELIVERY_AMBIGUOUS");
    expect(blindCalls).toBe(1);
    expect(blind.db.prepare("SELECT state, thread_id FROM execution_attempts WHERE origin = 'work_item'").get()).toEqual({ state: "prepared", thread_id: null });
  });

  it("refuses foreign, multiple, incomplete, wrong-parent, and wrong-idempotency evidence without retry or premature terminalization", async () => {
    for (const name of ["foreign project", "multiple exact", "incomplete", "wrong parent", "wrong idempotency"]) {
    const fixture = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    const request = transitionRequest(fixture.fenceToken, "in_progress", 2);
    const spawn = dispatchSpawn(fixture.orchestratorThreadId);
    const marker = `[dispatch:${request.idempotencyKey}]`;
    const threads = name === "foreign project"
      ? [makeThreadResponse({ id: "foreign", projectId: "project-foreign", title: `lane ${marker}` })]
      : name === "multiple exact"
        ...
        : name === "incomplete"
          ? [Object.assign(makeThreadResponse({ id: "incomplete", projectId: PROJECT_ID }), { title: undefined })]
          : name === "wrong parent"
            ? [makeThreadResponse({ id: "wrong-parent", projectId: PROJECT_ID, parentThreadId: "other-orchestrator", title: `lane ${marker}` })]
            : [makeThreadResponse({ id: "wrong-id", projectId: PROJECT_ID, parentThreadId: fixture.orchestratorThreadId, title: "lane [dispatch:other-id]" })];
    let calls = 0;
    fixture.host.harness.sdk.stub("threads.spawn", (async () => { calls += 1; throw new Error("response ambiguous"); }) as never);
  ...
  it("recovers an existing prepared attempt through the same governed dispatch seam", async () => {
    const fixture = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    const request = transitionRequest(fixture.fenceToken, "in_progress", 2);
    const spawn = dispatchSpawn(fixture.orchestratorThreadId);
    ...
    fixture.host.harness.sdk.stub("threads.list", (async () => {
      listCalls += 1;
      if (listCalls === 1) throw new Error("recovery read interrupted");
      return [];
    }) as never);
    fixture.host.harness.sdk.stub("threads.spawn", (async (input: { title?: string }) => {
      spawnCalls += 1;
      if (spawnCalls === 1) throw new Error("native refusal before create");
      return makeThreadResponse({ id: "prepared-recovered", projectId: PROJECT_ID, parentThreadId: fixture.orchestratorThreadId, title: input.title ?? null });
    }) as never);
    const first = JSON.parse(await fixture.host.harness.callAgentTool("dispatch_lane", { request, spawn }, { projectId: PROJECT_ID, threadId: fixture.orchestratorThreadId }) as string);
    expect(first.outcome).toBe("EXTERNAL_DELIVERY_AMBIGUOUS");
    expect(fixture.db.prepare("SELECT state, thread_id FROM execution_attempts WHERE origin = 'work_item'").get()).toEqual({ state: "prepared", thread_id: null });
    const second = JSON.parse(await fixture.host.harness.callAgentTool("dispatch_lane", { request, spawn }, { projectId: PROJECT_ID, threadId: fixture.orchestratorThreadId }) as string);
  ...
  it("refuses a new-format prepared dispatch replay whose title changed", async () => {
    const fixture = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    const request = transitionRequest(fixture.fenceToken, "in_progress", 2);
    const originalSpawn = dispatchSpawn(fixture.orchestratorThreadId);
    const changedSpawn = { ...originalSpawn, title: "changed" };
    const { threadId: _threadId, ...intentAttempt } = request.workAttempt!;
    expect(applyWithFixtureReceipt(fixture.db, {
      ...request,
      workAttempt: intentAttempt,
      reasonCode: `dispatch_parent:${fixture.orchestratorThreadId}:title=${encodeURIComponent(originalSpawn.title)}`,
    })).toMatchObject({ outcome: "OK" });
    expect(fixture.db.prepare("SELECT reason_code FROM execution_attempts WHERE origin = 'work_item'").get()).toEqual({ reason_code: `work_item_dispatch_intent:${request.idempotencyKey}:parent=${fixture.orchestratorThreadId}:title=lane` });
    const replayRequest = {
      ...request,
      workAttempt: intentAttempt,
      reasonCode: `dispatch_parent:${fixture.orchestratorThreadId}:title=${encodeURIComponent(changedSpawn.title)}`,
    };
    fixture.db.prepare("UPDATE mutation_receipts SET request_digest = ? WHERE project_id = ? AND idempotency_key = ?").run(
      mutationRequestDigest(parseApplyRequest(replayRequest)),
      PROJECT_ID,
      request.idempotencyKey,
    );
    fixture.host.harness.sdk.stub("threads.list", (async ({ archived }: { archived?: boolean }) => archived ? [] : [makeThreadResponse({
      id: "changed-replay-title",
      projectId: PROJECT_ID,
      parentThreadId: fixture.orchestratorThreadId,
      title: `changed [dispatch:${request.idempotencyKey}]`,
      status: "active",
    ...
    let spawnCalls = 0;
    fixture.host.harness.sdk.stub("threads.spawn", (async () => { spawnCalls += 1; throw new Error("title weakening must not bind or retry"); }) as never);
    const result = JSON.parse(await fixture.host.harness.callAgentTool("dispatch_lane", { request, spawn: changedSpawn }, { projectId: PROJECT_ID, threadId: fixture.orchestratorThreadId }) as string);
    expect(result.outcome).toBe("EXTERNAL_DELIVERY_AMBIGUOUS");
    expect(spawnCalls).toBe(0);
    expect(fixture.db.prepare("SELECT state, thread_id FROM execution_attempts WHERE origin = 'work_item'").get()).toEqual({ state: "prepared", thread_id: null });
  });

  it("recovers an exact legacy receipt after complete no-current-marker evidence", async () => {
    const fixture = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    const request = transitionRequest(fixture.fenceToken, "in_progress", 2);
    const spawn = dispatchSpawn(fixture.orchestratorThreadId);
    const { threadId: _threadId, ...intentAttempt } = request.workAttempt!;
    const legacyRequest = { ...request, workAttempt: intentAttempt, reasonCode: `dispatch_parent:${fixture.orchestratorThreadId}` };
    expect(applyWithFixtureReceipt(fixture.db, legacyRequest)).toMatchObject({ outcome: "OK" });
    expect(fixture.db.prepare("SELECT reason_code FROM execution_attempts WHERE origin = 'work_item'").get()).toEqual({ reason_code: `work_item_dispatch_intent:${request.idempotencyKey}:parent=${fixture.orchestratorThreadId}` });
    expect(fixture.db.prepare("SELECT request_digest FROM mutation_receipts WHERE project_id = ? AND idempotency_key = ?").get(PROJECT_ID, request.idempotencyKey)).toEqual({ request_digest: mutationRequestDigest(parseApplyRequest(legacyRequest)) });
    const historicalChildren = [
      ...Array.from({ length: 68 }, (_, index) => makeThreadResponse({ id: `historical-active-${index}`, projectId: PROJECT_ID, parentThreadId: fixture.orchestratorThreadId, title: `historical-active-${index} [dispatch:orch-g5-wi-gh-${571 + index}-dispatch-v1]` })),
      ...Array.from({ length: 4 }, (_, index) => makeThreadResponse({ id: `historical-archived-${index}`, projectId: PROJECT_ID, parentThreadId: fixture.orchestratorThreadId, title: `historical-archived-${index} [dispatch:orch-g5-wi-gh-${639 + index}-dispatch-v1]`, archivedAt: 1 })),
    ];
    fixture.host.harness.sdk.stub("threads.list", (async ({ archived }: { archived?: boolean }) => historicalChildren.filter((thread) => archived ? thread.archivedAt !== null : thread.archivedAt === null)) as never);
    let spawnCalls = 0;
    fixture.host.harness.sdk.stub("threads.spawn", (async (input: { projectId: string; parentThreadId?: string; title?: string }) => {
  ...
  it("refuses every native routing-profile drift before replaying an exact legacy receipt", async () => {
    const changedValues = {
    ...
    const routingFields = Object.keys(changedValues) as Array<keyof typeof changedValues>;
    const cases = [
      ...routingFields.flatMap((field) => [
        { name: `${field} changed`, spawn: (base: Record<string, unknown>) => ({ ...base, [field]: changedValues[field] }) },
        { name: `${field} omitted`, spawn: (base: Record<string, unknown>) => { const copy = { ...base }; delete copy[field]; return copy; } },
      ]),
      ...routingFields.filter((field) => field !== "visibility").map((field) => ({
        name: `${field} client preference`,
        spawn: (base: Record<string, unknown>) => ({ ...base, executionInputSources: { [field]: "client-preference" } }),
      })),
    ];
    for (const testCase of cases) {
      ...
      const legacyRequest = { ...request, workAttempt: intentAttempt, reasonCode: `dispatch_parent:${fixture.orchestratorThreadId}` };
      expect(applyWithFixtureReceipt(fixture.db, legacyRequest)).toMatchObject({ outcome: "OK" });
      const replaySpawn = testCase.spawn(spawn);
      let spawnCalls = 0;
      fixture.host.harness.sdk.stub("threads.spawn", (async () => { spawnCalls += 1; throw new Error(`${testCase.name} must refuse`); }) as never);
      const result = JSON.parse(await fixture.host.harness.callAgentTool("dispatch_lane", { request, spawn: replaySpawn }, { projectId: PROJECT_ID, threadId: fixture.orchestratorThreadId }) as string);
      expect(result.outcome).not.toBe("OK");
      expect(spawnCalls).toBe(0);
      expect(fixture.db.prepare("SELECT state, thread_id, reason_code FROM execution_attempts WHERE origin = 'work_item'").get()).toEqual({ state: "prepared", thread_id: null, reason_code: `work_item_dispatch_intent:${request.idempotencyKey}:parent=${fixture.orchestratorThreadId}` });
      expect(fixture.db.prepare("SELECT request_digest FROM mutation_receipts WHERE project_id = ? AND idempotency_key = ?").get(PROJECT_ID, request.idempotencyKey)).toEqual({ request_digest: mutationRequestDigest(parseApplyRequest(legacyRequest)) });
    }
  });

  it("refuses a workspace-write profile without permission normalization", async () => {
    const fixture = await fleetWatchdogFixture(0, true, 1, false);
    expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
    const requestedProfile = { ...ROLE_PROFILE, permissionMode: "workspace-write" };
    const request = transitionRequest(fixture.fenceToken, "in_progress", 2, { workAttempt: { ...transitionRequest(fixture.fenceToken, "in_progress", 2).workAttempt!, requestedProfile } });
    const spawn = dispatchSpawn(fixture.orchestratorThreadId, { permissionMode: "accept-edits", executionInputSources: EXPLICIT_EXECUTION_INPUT_SOURCES });
    const { threadId: _threadId, ...intentAttempt } = request.workAttempt!;
    const legacyRequest = { ...request, workAttempt: intentAttempt, reasonCode: `dispatch_parent:${fixture.orchestratorThreadId}` };
    expect(applyWithFixtureReceipt(fixture.db, legacyRequest)).toMatchObject({ outcome: "OK" });
    ...
    fixture.host.harness.sdk.stub("threads.spawn", (async (input: { projectId: string; parentThreadId?: string; title?: string }) => {
      spawnCalls += 1;
      return makeThreadResponse({ id: "legacy-profile-recovered", projectId: input.projectId, parentThreadId: input.parentThreadId ?? null, title: input.title ?? null, status: "active" });
    }) as never);
    const result = JSON.parse(await fixture.host.harness.callAgentTool("dispatch_lane", { request, spawn }, { projectId: PROJECT_ID, threadId: fixture.orchestratorThreadId }) as string);
    expect(result.outcome).toBe("INVALID_INPUT");
    expect(spawnCalls).toBe(0);
  ...
  it("does not select legacy receipt compatibility for mismatched dispatch context", async () => {
    const cases = ["parent", "project", "work item", "idempotency", "profile", "lifecycle"] as const;
    for (const name of cases) {
      ...
      const legacyRequest = { ...request, workAttempt: intentAttempt, reasonCode: `dispatch_parent:${fixture.orchestratorThreadId}` };
      expect(applyWithFixtureReceipt(fixture.db, legacyRequest)).toMatchObject({ outcome: "OK" });
      const replayRequest = name === "work item"
        ? { ...request, workItemId: "other-work-item" }
        : name === "idempotency"
          ? { ...request, idempotencyKey: "other-idempotency" }
          : name === "profile"
            ? { ...request, workAttempt: { ...request.workAttempt!, requestedProfile: { ...ROLE_PROFILE, model: "other-model" } } }
            : name === "lifecycle"
              ? { ...request, lifecycleState: "ready" as const }
              : request;
      const replaySpawn = name === "parent"
        ? { ...spawn, parentThreadId: "other-orchestrator" }
        : name === "project"
          ? { ...spawn, projectId: "project-foreign" }
          : spawn;
      let spawnCalls = 0;
      fixture.host.harness.sdk.stub("threads.spawn", (async () => { spawnCalls += 1; throw new Error("mismatched replay must not spawn"); }) as never);
      const result = JSON.parse(await fixture.host.harness.callAgentTool("dispatch_lane", { request: replayRequest, spawn: replaySpawn }, { projectId: PROJECT_ID, threadId: fixture.orchestratorThreadId }) as string);
      expect(result.outcome).not.toBe("OK");
      expect(spawnCalls).toBe(0);
      expect(fixture.db.prepare("SELECT state, thread_id FROM execution_attempts WHERE origin = 'work_item'").get()).toEqual({ state: "prepared", thread_id: null });
    }
  ...
  it("refuses legacy prepared recovery on archived, marker mismatch, foreign, or multiple marker evidence", async () => {
    const cases = ["archived", "marker mismatch", "foreign parent", "foreign project", "multiple"] as const;
    for (const name of cases) {
      const fixture = await fleetWatchdogFixture(0, true, 1, false);
      expect(applyWithFixtureReceipt(fixture.db, transitionRequest(fixture.fenceToken, "ready", 1))).toMatchObject({ outcome: "OK" });
      const request = transitionRequest(fixture.fenceToken, "in_progress", 2);
      ...
      const threads = name === "archived"
        ...
        : name === "marker mismatch"
          ...
          : name === "foreign parent"
            ? [makeThreadResponse({ id: "foreign-parent", projectId: PROJECT_ID, parentThreadId: "other-orchestrator", title: `lane ${marker}` })]
            : name === "foreign project"
              ? [makeThreadResponse({ id: "foreign-project", projectId: "project-foreign", parentThreadId: fixture.orchestratorThreadId, title: `lane ${marker}` })]
              : [makeThreadResponse({ id: "one", projectId: PROJECT_ID, parentThreadId: fixture.orchestratorThreadId, title: `lane ${marker}` }), makeThreadResponse({ id: "two", projectId: PROJECT_ID, parentThreadId: fixture.orchestratorThreadId, title: `lane ${marker}` })];
      fixture.host.harness.sdk.stub("threads.list", (async ({ archived }: { archived?: boolean }) => threads.filter((thread) => archived ? thread.archivedAt !== null : thread.archivedAt === null)) as never);
      let spawnCalls = 0;
...
`);
  ...
  it("preserves a failed-delivery operator message while appending archive state", () => {
    const db = new Database(":memory:");
    databaseIsReady(db);
    try {
    for (const statement of MIGRATIONS.slice(0, -20)) db.exec(statement);
  ...
  it("appends authority-root schema and bumps the runtime contract", () => {
    ...
    expect(MIGRATIONS[39]).toContain("ALTER TABLE operator_messages ADD COLUMN archived_at_ms");
    expect(MIGRATIONS[40]).toContain("CREATE TABLE IF NOT EXISTS bootstrap_derivation_receipts");
    ...
    expect(cachedConsumerRolloutEvidence(cachedConsumerObservations(35, 30))).toMatchObject({ action: "reread", verified: 4 });
  ...
  it("appends the review_pending work-item migration without rewriting historical rows", () => {
    ...
    try {
      ...
      const priorStatementDigests = MIGRATIONS.slice(0, -18).map(sha256);
      db.transaction(() => db.exec(MIGRATIONS.at(-19)!))();
      expect(MIGRATIONS.slice(0, -18).map(sha256)).toEqual(priorStatementDigests);
  ...
  it("preserves execution-attempt records and SQLite integrity when appending review linkage columns", () => {
    ...
    try {
      db.transaction(() => { for (const statement of MIGRATIONS.slice(0, -16)) db.exec(statement); })();
      const configJson = "{}";
  ...
  it("assembles the production v22 cached-consumer rollout receipt with stale-v21 refusal semantics", async () => {
    ...
    expect(JSON.parse(evidence.durableRefJson)).toMatchObject({
      reread: { observations: CACHED_CONSUMERS.map((name) => ({ name, observedSchemaVersion: 35, observedContractVersion: 30 })), action: "reread", expected: 4, attempted: 4, verified: 4 },
  ...
  it("atomically rebuilds attempts with populated production child FKs and rejects the parent-drop mutant", () => {
    ...
    const decisionDigest = () => sha256(canonicalJson(db.prepare("SELECT decision_id, project_id, decision_identity_digest FROM decisions ORDER BY decision_id").all()));
    ...
    try {
      ...
      const before = {
        counts: Object.fromEntries(childTables.map((table) => [table, (db.prepare(`SELECT COUNT(*) AS count FROM ${table}`).get() as { count: number }).count])),
        digests: Object.fromEntries(childTables.map((table) => [table, tableDigest(table)])),
      ...
      expect({ decisionDigest: decisionDigest(), dispositionDigest: dispositionDigest() }).toEqual({ decisionDigest: before.decisionDigest, dispositionDigest: before.dispositionDigest });
      const parentDropMutant = MIGRATIONS.map((statement, id) => id === GH636_PREVIOUS_MIGRATION_ID
        ? statement.replace(
          "   ALTER TABLE execution_attempts_gh624 RENAME TO execution_attempts;",
          "   DROP TABLE execution_attempts_gh624_old;\n   ALTER TABLE execution_attempts_gh624 RENAME TO execution_attempts;",
        )
        : statement);
      expect(() => sdkMigrate(db, parentDropMutant)).toThrow(/FOREIGN KEY constraint failed|no such table: execution_attempts_gh624_old/iu);
      ...
      expect(reopened.prepare("SELECT COUNT(*) AS count FROM decision_evidence").get()).toEqual({ count: before.counts.decision_evidence });
      expect(sha256(canonicalJson(reopened.prepare("SELECT decision_id, project_id, decision_identity_digest FROM decisions ORDER BY decision_id").all()))).toBe(before.decisionDigest);
      expect(sha256(canonicalJson(reopened.prepare("SELECT decision_id, disposition_sequence, disposition, actor_receipt_id FROM decision_dispositions ORDER BY decision_id, disposition_sequence").all()))).toBe(before.dispositionDigest);
      reopened.close();
    } finally {
      if (db.open) db.close();
      rmSync(directory, { recursive: true, force: true });
  ...
  it("preserves every varying non-null prior execution_attempts value across the appended rebuild", () => {
    ...
    try {
      ...
      db.transaction(() => db.exec(MIGRATIONS.at(-20)!))();
  ...
  it("recognizes a ledgered v31 success without replaying the repair and refuses an unledgered v31", () => {
    const makeDatabase = (ledgerThrough: number, schema31Applied = false) => {
      const database = new Database(":memory:");
      databaseIsReady(database);
      database.transaction(() => {
        for (const statement of MIGRATIONS.slice(0, GH636_PREVIOUS_MIGRATION_ID)) database.exec(statement);
        if (schema31Applied) database.exec(MIGRATIONS[GH636_PREVIOUS_MIGRATION_ID]!);
      })();
    ...
    const succeeded = makeDatabase(GH636_PREVIOUS_MIGRATION_ID, true);
    try {
      const executed: number[] = [];
      migrateCanonicalStore(succeeded, (database, statements) => sdkMigrate(database, statements, executed));
      expect(executed).toEqual([GH636_REPAIR_MIGRATION_ID, GH637_DOMAIN_MIGRATION_ID, GH644_LOCAL_CANDIDATE_REVIEW_MIGRATION_ID, GH644_LOCAL_REVIEW_AUTHORITY_MIGRATION_ID]);
      expect(succeeded.prepare("SELECT id FROM _bb_migrations ORDER BY id").all()).toEqual(
        Array.from({ length: GH644_LOCAL_REVIEW_AUTHORITY_MIGRATION_ID + 1 }, (_, id) => ({ id })),
      );
    ...
    const ambiguous = makeDatabase(GH636_PREVIOUS_MIGRATION_ID - 1, true);
    try {
      const executed: number[] = [];
    ...
    try {
      databaseIsReady(partial);
      partial.exec("CREATE TABLE _bb_migrations (id INTEGER PRIMARY KEY, applied_at INTEGER NOT NULL)");
      partial.prepare("INSERT INTO _bb_migrations (id, applied_at) VALUES (0, 1)").run();
      const executed: number[] = [];
      expect(() => migrateCanonicalStore(partial, (database, statements) => sdkMigrate(database, statements, executed))).toThrow(/prior migration ledger is incomplete/iu);
      expect(executed).toEqual([]);
      expect(partial.prepare("SELECT MAX(id) AS id FROM _bb_migrations").get()).toEqual({ id: 0 });
      expect(partial.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'execution_attempts'").get()).toBeUndefined();
    } finally {
      partial.close();
    }

    const skippedAppend = makeDatabase(GH636_PREVIOUS_MIGRATION_ID, true);
    try {
      expect(() => migrateCanonicalStore(skippedAppend, (database, statements) => sdkMigrate(database, statements.slice(0, GH636_REPAIR_MIGRATION_ID), []))).toThrow(/ledger is incomplete/iu);
      expect(skippedAppend.prepare("SELECT MAX(id) AS id FROM _bb_migrations").get()).toEqual({ id: GH636_PREVIOUS_MIGRATION_ID });
    } finally {
      skippedAppend.close();
    }
  });
  ...
  it("applies the appended domain migration when both GH636 statements are already ledgered", () => {
    ...
    try {
      ...
      expect(executed).toEqual([GH637_DOMAIN_MIGRATION_ID, GH644_LOCAL_CANDIDATE_REVIEW_MIGRATION_ID, GH644_LOCAL_REVIEW_AUTHORITY_MIGRATION_ID]);
  ...
  it("runs the closed lifecycle with exact replay/CAS, canonical export/import, and target-safe retirement", async () => {
    ...
    expect(firstExport.manifest).toMatchObject({ schemaVersion: 35, schemaDigest, contractVersion: 30, contractDigest });
  ...
  it("authorizes only the exact governorship-rooted cross-project bootstrap Decision", async () => {
    ...
    const decision = {
      decisionId: "cross-project-authority",
      repoTargetId: null,
    ...
    const create = applyWithFixtureReceipt(db, decisionCreateRequest(fenceToken, decision.decisionId, {
      actorReceiptId: "bootstrap-plugin", repoTargetId: null, decision,
    }));
    expect(create).toMatchObject({ outcome: "OK", currentResourceRevision: 1 });
    const stored = db.prepare("SELECT authority_root_json, authority_root_digest FROM decisions WHERE decision_id = ?").get(decision.decisionId) as { authority_root_json: string; authority_root_digest: string };
    expect(stored.authority_root_digest).toBe(sha256(stored.authority_root_json));
    expect(JSON.parse(stored.authority_root_json)).toMatchObject({ projectId: PROJECT_ID, governanceEpoch: 1, actorReceiptId: "bootstrap-plugin" });
    const adoptRequest = decisionDispositionRequest(fenceToken, decision.decisionId, 1, {
      actorReceiptId: "bootstrap-plugin", repoTargetId: null,
    });
    ...
    const switchedDecision = {
      ...decision,
      decisionId: "cross-project-root-switch",
    };
    expect(applyWithFixtureReceipt(db, decisionCreateRequest(fenceToken, switchedDecision.decisionId, {
      actorReceiptId: "bootstrap-plugin", repoTargetId: null, decision: switchedDecision,
    })).outcome).toBe("OK");
    expect(applyWithFixtureReceipt(db, {
      projectId: PROJECT_ID,
      operationClass: "governor_claim",
      idempotencyKey: "cross-project-root-switch-claim",
      actorReceiptId: "bootstrap-plugin",
      expectedConfigRevision: 1,
      expectedGovernanceEpoch: 1,
      expectedFenceToken: fenceToken,
      repoTargetId: null,
    }).outcome).toBe("OK");
    expect(applyWithFixtureReceipt(db, decisionDispositionRequest(fenceToken, switchedDecision.decisionId, 1, {
      actorReceiptId: "bootstrap-plugin", repoTargetId: null, idempotencyKey: "cross-project-root-switch-stale-disposition",
  ...
  it("kills bootstrap authority mutants and preserves ordinary role authority", async () => {
    ...
    const mutants = [
      { name: "wrong-actor", actorReceiptId: "wrong-plugin", decision: { ...exact } },
      { name: "wrong-source", actorReceiptId: "bootstrap-plugin", decision: { ...exact, scope: { ...exact.scope, sourceProjectId: "proj_other" } } },
      { name: "same-target", actorReceiptId: "bootstrap-plugin", decision: { ...exact, scope: { ...exact.scope, targetProjectId: PROJECT_ID } } },
      { name: "extra-scope", actorReceiptId: "bootstrap-plugin", decision: { ...exact, scope: { ...exact.scope, extra: true } } },
      { name: "extra-options", actorReceiptId: "bootstrap-plugin", decision: { ...exact, options: { ...exact.options, extra: true } } },
      { name: "wrong-class", actorReceiptId: "bootstrap-plugin", decision: { ...exact, decisionClass: "legacy_adoption" as const } },
    ];
    ...
    expect(applyWithFixtureReceipt(db, decisionCreateRequest(fenceToken, "cross-project-foreign-request-target", {
      ...
      decision: { decisionId: "cross-project-foreign-request-target", ...exact },
    })).outcome).toBe("REPO_TARGET_FOREIGN");
    expect(applyWithFixtureReceipt(db, decisionCreateRequest(fenceToken, "cross-project-stale-fence", {
    ...
    db.prepare(
      `INSERT INTO decisions (decision_id, project_id, config_revision, repo_target_id, scope_json, scope_digest, current_resource_revision, decision_class, options_json, decision_identity_digest)
       VALUES ('legacy-bootstrap-authority', ?, 1, NULL, ?, ?, 1, 'operator_only', ?, ?)`,
    ).run(PROJECT_ID, legacyScope, sha256(legacyScope), canonicalJson(exact.options), sha256(canonicalJson({ projectId: PROJECT_ID, configRevision: 1, repoTargetId: null, scope: exact.scope, decisionClass: "operator_only", options: exact.options })));
  ...
  it("reports deterministic v5 Decision and evidence integrity without adopting or repairing state", async () => {
    ...
    expect(first).toMatchObject({
      ...
      evidence: {
        ...
        cachedConsumers: { oldSchemaVersion: 32, newSchemaVersion: 35, action: "unknown", expected: 4, attempted: 0, verified: 0 },
        schema: { version: 35 },
      },
    });
    expect(exportFoundation(db, PROJECT_ID)).toEqual(before);
  ...
  it("recovers a blocked WorkItem through an already-succeeded immutable successor", async () => {
    const host = await loadedHost();
    const { db, fenceToken } = seedAndBootstrap(host);
    const create = (workItemId: string, issueNumber?: number) => applyWithFixtureReceipt(db, workItemCreateRequest(fenceToken, {
      idempotencyKey: `${workItemId}-create`,
      workItemId,
      workItem: {
        workItemId,
        title: workItemId,
        body: workItemId,
        ...(issueNumber === undefined ? {} : { githubIssue: { issueNumber } }),
      },
    }));
    ...
    const targetId = "wi-gh-643";
    const predecessorId = "wi-gh-637";
    const replacementId = "wi-gh-637-r11";
    expect(create(targetId, 643).outcome).toBe("OK");
    expect(create(predecessorId).outcome).toBe("OK");
    expect(create(replacementId).outcome).toBe("OK");
    expect(transition(targetId, "ready", 1).outcome).toBe("OK");
    expect(transition(targetId, "blocked", 2, {
      workItemWait: { kind: "work_item_succeeded", workItemId: predecessorId, declaredBySeat: "worker-seat" },
    }).outcome).toBe("OK");
    expect(transition(predecessorId, "cancelled", 1).outcome).toBe("OK");
    expect(transition(replacementId, "ready", 1).outcome).toBe("OK");
    expect(transition(replacementId, "in_progress", 2).outcome).toBe("OK");
    expect(transition(replacementId, "review_pending", 3).outcome).toBe("OK");
    expect(transition(replacementId, "succeeded", 4).outcome).toBe("OK");

    const swapRequest = transitionRequest(fenceToken, "blocked", 3, {
      idempotencyKey: "gh650-fired-successor-rebind",
      workItemId: targetId,
      workItemWait: { kind: "work_item_succeeded", workItemId: replacementId, declaredBySeat: "worker-seat", note: "Use the live current-revision prerequisite." },
      workItemUnblock: { kind: "work_item_succeeded", workItemId: predecessorId },
    });
    const swap = applyWithFixtureReceipt(db, swapRequest);
    expect(swap).toMatchObject({ outcome: "OK", currentResourceRevision: 4 });
    expect(db.prepare("SELECT lifecycle_state, resource_revision FROM work_items WHERE work_item_id = ?").get(targetId)).toEqual({ lifecycle_state: "ready", resource_revision: 4 });
    expect(db.prepare("SELECT 1 FROM work_item_waits WHERE work_item_id = ?").get(targetId)).toBeUndefined();
    expect(db.prepare("SELECT owner, repo, issue_number FROM external_work_refs WHERE work_item_id = ?").get(targetId)).toEqual({ owner: GITHUB_OWNER, repo: GITHUB_REPO, issue_number: 643 });
    const event = db.prepare(
      "SELECT aggregate_revision, event_type, actor_receipt_id, event_json FROM state_events WHERE aggregate_id = ? ORDER BY aggregate_revision DESC LIMIT 1",
    ).get(targetId) as { aggregate_revision: number; event_type: string; actor_receipt_id: string; event_json: string };
    expect(event).toMatchObject({ aggregate_revision: 4, event_type: "work_item_wait_swapped", actor_receipt_id: RECEIPT_ID });
    expect(JSON.parse(event.event_json)).toMatchObject({
      from: "blocked",
      to: "ready",
      previousBlocker: { kind: "work_item_succeeded", workItemId: predecessorId },
      replacementBlocker: { kind: "work_item_succeeded", workItemId: replacementId },
      unblock: { kind: "work_item_succeeded", workItemId: predecessorId },
      blocker: { kind: "work_item_succeeded", workItemId: replacementId },
    });
    const recovered = exportFoundation(db, PROJECT_ID);
    expect(applyWithFixtureReceipt(db, swapRequest)).toMatchObject({ outcome: "OK" });
    expect(exportFoundation(db, PROJECT_ID)).toEqual(recovered);
  });

  it("refuses a blocker cycle before mutation", async () => {
    const host = await loadedHost();
    const { db, fenceToken } = seedAndBootstrap(host);
    const create = (workItemId: string) => applyWithFixtureReceipt(db, workItemCreateRequest(fenceToken, {
      idempotencyKey: `${workItemId}-create`, workItemId,
    ...
    expect(create("cycle-a").outcome).toBe("OK");
    expect(create("cycle-b").outcome).toBe("OK");
    expect(transition("cycle-a", "ready", 1).outcome).toBe("OK");
    expect(transition("cycle-b", "ready", 1).outcome).toBe("OK");
    expect(transition("cycle-b", "blocked", 2, {
      workItemWait: { kind: "work_item_succeeded", workItemId: "cycle-a", declaredBySeat: "worker-seat" },
    }).outcome).toBe("OK");
    const before = exportFoundation(db, PROJECT_ID);
    expect(transition("cycle-a", "blocked", 2, {
      idempotencyKey: "cycle-a-blocked-on-b",
  ...
  it("migrates v3 role evidence to v4 without manufacturing a canonical holder attempt", async () => {
    ...
    for (const statement of MIGRATIONS.at(-15)!.split(";").filter((statement) => statement.includes("ALTER TABLE execution_attempts"))) db.exec(statement);
    ...
    expect(cachedConsumerRolloutEvidence(cachedConsumerObservations(11, 19))).toMatchObject({ oldSchemaVersion: 32, newSchemaVersion: 35, oldContractVersion: 27, newContractVersion: 30, action: "refused", expected: 4, attempted: 4, verified: 0 });
    expect(cachedConsumerRolloutEvidence(cachedConsumerObservations(12, 19))).toMatchObject({ oldSchemaVersion: 32, newSchemaVersion: 35, oldContractVersion: 27, newContractVersion: 30, action: "refused", expected: 4, attempted: 4, verified: 0 });
    expect(cachedConsumerRolloutEvidence(cachedConsumerObservations(22, 22))).toMatchObject({ oldSchemaVersion: 32, newSchemaVersion: 35, oldContractVersion: 27, newContractVersion: 30, action: "refused", expected: 4, attempted: 4, verified: 0 });
    expect(cachedConsumerRolloutEvidence(cachedConsumerObservations(35, 30))).toMatchObject({ oldSchemaVersion: 32, newSchemaVersion: 35, oldContractVersion: 27, newContractVersion: 30, action: "reread", expected: 4, attempted: 4, verified: 4 });
  });
