...
describe("bb-collab plugin boundary", () => {
  ...
  it("registers a second synthetic project through the strict seam and preserves tenant isolation", async () => {
    ...
    expect(applyFixtureMutation(db, decisionCreateRequest(sourceFence, "source-bootstrap-decision", {
      actorReceiptId: sourceRoleActor,
      expectedGovernanceEpoch: 2,
      expectedFenceToken: sourceFence,
      idempotencyKey: "create-source-bootstrap-decision",
    }))).toMatchObject({ outcome: "OK" });
    expect(applyFixtureMutation(db, decisionDispositionRequest(sourceFence, "source-bootstrap-decision", 1, {
      actorReceiptId: sourceRoleActor,
      expectedGovernanceEpoch: 2,
      expectedFenceToken: sourceFence,
      idempotencyKey: "adopt-source-bootstrap-decision",
    }))).toMatchObject({ outcome: "OK" });
    ...
    expect(db.prepare("SELECT project_id, genesis_receipt_id, source_project_id, source_governance_epoch, authorizing_decision_id, authorizing_disposition_sequence FROM bootstrap_derivation_receipts WHERE project_id = ?").get(secondProject)).toEqual({
...
`);
  ...
  it("appends operator inbox archive schema without bumping runtime contract v22", () => {
    ...
    expect(sha256(MIGRATIONS.slice(0, -1).join("\n"))).toBe("27f336d0a98e5d5007ee1250f8bf4193f9e7bc17eb1b90d965bc7cac053bdca6");
    expect(schemaDigest).toBe("bf7fc787a2e44193f0523c640f2ca57d4e56ee7e5366b89d7ef3b577b85bce88");
    expect(MIGRATIONS.at(-2)).toContain("ALTER TABLE operator_messages ADD COLUMN archived_at_ms");
    expect(MIGRATIONS.at(-1)).toContain("CREATE TABLE IF NOT EXISTS bootstrap_derivation_receipts");
    ...
    expect(cachedConsumerRolloutEvidence(cachedConsumerObservations(28, 22))).toMatchObject({ action: "reread", verified: 4 });
  ...
  it("assembles the production v22 cached-consumer rollout receipt with stale-v21 refusal semantics", async () => {
    ...
    expect(JSON.parse(evidence.durableRefJson)).toMatchObject({
      reread: { observations: CACHED_CONSUMERS.map((name) => ({ name, observedSchemaVersion: 28, observedContractVersion: 22 })), action: "reread", expected: 4, attempted: 4, verified: 4 },
  ...
  it("runs the closed lifecycle with exact replay/CAS, canonical export/import, and target-safe retirement", async () => {
    ...
    expect(firstExport.manifest).toMatchObject({ schemaVersion: 28, schemaDigest, contractVersion: 22, contractDigest });
  ...
  it("reports deterministic v5 Decision and evidence integrity without adopting or repairing state", async () => {
    ...
    expect(first).toMatchObject({
      ...
      evidence: {
        ...
        cachedConsumers: { oldSchemaVersion: 27, newSchemaVersion: 28, action: "unknown", expected: 4, attempted: 0, verified: 0 },
        schema: { version: 28 },
      },
    });
    expect(exportFoundation(db, PROJECT_ID)).toEqual(before);
  ...
  it("migrates v3 role evidence to v4 without manufacturing a canonical holder attempt", async () => {
    ...
    for (const statement of MIGRATIONS.at(-8)!.split(";").filter((statement) => statement.includes("ALTER TABLE execution_attempts"))) db.exec(statement);
    ...
    expect(cachedConsumerRolloutEvidence(cachedConsumerObservations(11, 19))).toMatchObject({ oldSchemaVersion: 27, newSchemaVersion: 28, oldContractVersion: 21, newContractVersion: 22, action: "refused", expected: 4, attempted: 4, verified: 0 });
    expect(cachedConsumerRolloutEvidence(cachedConsumerObservations(12, 19))).toMatchObject({ oldSchemaVersion: 27, newSchemaVersion: 28, oldContractVersion: 21, newContractVersion: 22, action: "refused", expected: 4, attempted: 4, verified: 0 });
    expect(cachedConsumerRolloutEvidence(cachedConsumerObservations(22, 22))).toMatchObject({ oldSchemaVersion: 27, newSchemaVersion: 28, oldContractVersion: 21, newContractVersion: 22, action: "refused", expected: 4, attempted: 4, verified: 0 });
    expect(cachedConsumerRolloutEvidence(cachedConsumerObservations(28, 22))).toMatchObject({ oldSchemaVersion: 27, newSchemaVersion: 28, oldContractVersion: 21, newContractVersion: 22, action: "reread", expected: 4, attempted: 4, verified: 4 });
  });
