...
`);
  ...
  it.each([
    ...
    ["unreachable candidate", (_attempt: Record<string, unknown>, fixture: Awaited<ReturnType<typeof assignmentFixture>>) => { fixture.host.harness.sdk.stub("environments.status", (async () => ({ outcome: "unavailable" })) as never); return _attempt; }],
    ...
    const tableDigest = (table: string) => {
      const columns = (db.prepare(`PRAGMA table_info(${table})`).all() as Array<{ name: string }>).map(({ name }) => name)
        ...
        .filter((name) => table !== "execution_attempts" || ![
          ...
          "review_candidate_kind", "review_candidate_json",
        ].includes(name));
