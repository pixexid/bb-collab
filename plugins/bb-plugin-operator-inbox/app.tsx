...
function formatTime(timestamp: number): string { return new Date(timestamp).toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" }); }
...
function senderLabel(message: OperatorMessage): string { return asText(message.senderTitle) ?? asText(message.senderThreadId) ?? "Sender unavailable"; }
    ...
    {sidebar.projects.length === 0 ? <section className="rounded-lg border border-dashed border-border p-6 text-center"><h2 className="font-medium">No projects available</h2><p className="mt-1 text-sm text-muted-foreground">A registered project is required before operator messages can appear here.</p></section> : <section aria-labelledby="inbox-list-heading" className="grid min-w-0 gap-3 md:grid-cols-[minmax(16rem,0.42fr)_minmax(0,1fr)]">
      <div className="min-w-0 overflow-hidden rounded-lg border border-border"><div className="flex flex-wrap items-baseline justify-between gap-2 border-b border-border bg-muted/10 px-3 py-3"><div><h2 id="inbox-list-heading" className="font-semibold">Messages</h2><p className="text-xs text-muted-foreground">{currentProjectLabel}</p></div><span className="text-xs text-muted-foreground">{messages.length} {messages.length === 1 ? "message" : "messages"}</span></div>
        ...
        <div role="list" aria-label="Operator messages">{visibleMessages.map((message) => {
          ...
          return <article key={key} role="listitem" className={`border-b border-border last:border-b-0 ${selected ? "bg-primary/5" : "bg-transparent"}`}>
            <button type="button" aria-pressed={selected} aria-label={`${selected ? "Selected. " : "Select "}message from ${sender}. ${projectNames.get(message.projectId) ?? message.projectId}. ${severityLabel(message.severity)}. ${stateLabel(message)}. ${formatTime(message.createdAtMs)}`} style={{ textAlign: "left", width: "100%" }} onClick={() => setSelectedMessageKey(key)} className={`grid min-w-0 gap-2 px-3 py-3 transition-colors duration-150 hover:bg-muted/60 active:bg-muted/80 focus-visible:z-10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary motion-reduce:transition-none ${selected ? "border-l-2 border-primary pl-[0.625rem]" : "border-l-2 border-transparent"}`}>
              <span className="flex min-w-0 items-start gap-2"><span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${message.readAtMs === null ? "bg-primary" : "bg-transparent"}`} aria-label={message.readAtMs === null ? "Unread" : undefined} /><span className={`min-w-0 flex-1 break-words text-sm ${message.readAtMs === null ? "font-semibold" : "font-medium"}`}>{sender}</span><time className="shrink-0 text-xs text-muted-foreground" dateTime={new Date(message.createdAtMs).toISOString()} title={formatTime(message.createdAtMs)} aria-label={`Received ${formatTime(message.createdAtMs)}`}>{formatRelativeTime(message.createdAtMs)}</time></span>
      ...
      {selectedMessage ? <article aria-labelledby="selected-message-heading" className="min-w-0 rounded-lg border border-border bg-background"><header className="grid gap-3 border-b border-border bg-muted/10 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Selected message</p><h2 id="selected-message-heading" className="mt-1 break-words text-lg font-semibold">{selectedSenderId ? <a href="#" className="underline decoration-muted-foreground/50 underline-offset-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary" aria-label={`Open sender session ${senderLabel(selectedMessage)}`} onClick={(event) => { event.preventDefault(); navigate.toThread(selectedSenderId); }}>{senderLabel(selectedMessage)}</a> : senderLabel(selectedMessage)}</h2><p className="mt-1 break-words text-sm text-muted-foreground">{selectedProjectLabel} · {severityLabel(selectedMessage.severity)} · <time dateTime={new Date(selectedMessage.createdAtMs).toISOString()} title={formatTime(selectedMessage.createdAtMs)} aria-label={`Received ${formatTime(selectedMessage.createdAtMs)}`}>{formatRelativeTime(selectedMessage.createdAtMs)}</time></p></div><div className="flex flex-wrap gap-2 text-xs"><span className={`rounded-full px-2.5 py-1 font-medium ${selectedMessage.readAtMs === null ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>{selectedMessage.readAtMs === null ? "Unread" : "Read"}</span>{selectedMessage.archivedAtMs != null ? <span className="rounded-full bg-muted px-2.5 py-1 font-medium text-muted-foreground">Archived</span> : null}{deliveryLabel(selectedMessage) ? <span className={`rounded-full px-2.5 py-1 font-medium ${deliveryLabel(selectedMessage) === "Delivery failed" ? "bg-destructive/10 text-destructive" : "bg-muted text-muted-foreground"}`}>{deliveryLabel(selectedMessage)}</span> : null}</div></div><div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">{asText(selectedMessage.senderLaneId) ? <span>Lane: {asText(selectedMessage.senderLaneId)}</span> : null}</div></header>
