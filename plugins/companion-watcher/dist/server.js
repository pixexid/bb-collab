...
async function github(repo) {
  ...
  const green = Array.isArray(prs) ? prs.flatMap((x) => {
    ...
    const ok = Array.isArray(checks) && checks.length > 0 && checks.every((c) => c && typeof c === "object" && ["SUCCESS", "NEUTRAL", "SKIPPED"].includes(String(c.conclusion)));
    return ok ? [{ number: x.number, green: true }] : [];
  }) : [];
...
function companionWatcher(bb) {
  ...
  bb.events.on("thread.idle", async ({ thread }) => {
    ...
    try {
      const target = store.prepare(`SELECT thread_id, project_id FROM execution_attempts WHERE thread_id=? AND origin='role_holder' AND role_id='project-orchestrator'`).get(thread.id);
      if (!target) return;
